...
export default {
	...
	methods: {
			...
			await this.$api.http
				...
				.then(res => {
					...
					this.shareData = {
						...
						shareTitle: this.$api.shopSetting.product_share_title,
		...
		sharePath() {
			...
			// #ifdef MP
			url = `${this.$api.routesConfig.product.path}?id=${this.productId}`;
			// #endif
			// #ifndef MP
			url = `${domain}${this.$api.routesConfig.product.path}?id=${this.productId}`;
			// #endif
			if (this.$api.store.getters.hasLogin) {
