<template>
	<view class="dz-tips" :class="['dz-' + type, isShow ? 'dz-tip-show' : '']" :style="{
		top: navbarHeight + 'px',
		zIndex: mZIndex
	}">{{ title }}</view>
</template>
...
<script>
	 ...
	 * @example <dz-top-tips ref="uTips"></dz-top-tips>
	 */
	export default {
		name: "dz-top-tips",
		props: {
		...
		computed: {
			mZIndex() {
				return this.zIndex ? this.zIndex : this.$api.zIndex.topTips;
			}
		},
		methods: {
			show(config = {}) {
