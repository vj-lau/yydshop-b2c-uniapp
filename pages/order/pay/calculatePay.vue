	...
	<view class="pay-content">
		...
		<dz-popup v-model="allinpaySchemeShow" mode="center" width="90%" border-radius="30" :closeable="true">
			<view class="dz-p-t-40 dz-p-b-40 dz-p-l-20 dz-p-r-20">
				...
				<a target="_blank" class="link" :href="allinpaySchemeAlipay" @tap="schemeAlipaySchemeAlipayClick">
					<dz-button
						:custom-style="{
							background: theme.dzBaseColor,
							color: theme.dzBaseFontColor,
...
export default {
	...
	async onLoad(e) {
		this.orderGroup = e.orderGroup || 'fortune';
		this.apiUrl = this.$api.apiShop.calculatePay;
		this.queryForm = e.params;
	},
	onShow() {},
	methods: {
		quickClick(index) {
		...
		async initOrder() {
			// 获取测算配置
			await this.$api.http
				.post(this.$api.apiShop.calculatePayConfirm, {
					id: this.id
				})
				.then(res => {
					this.orderGroup = 'fortune';
					this.fortune_globe_pay = res.data.config.fortune_globe_pay;
					if (res.data.config.fortune_globe_pay == 0) {
						this.money = res.data.subject.price;
					} else if (res.data.config.fortune_globe_pay == 1) {
						this.money = res.data.config.fortune_globe_money;
						this.orderGroup = 'fortune_globe';
					} else {
						this.money = this.fortuneGlobe[0].price = res.data.subject.price;
						this.fortuneGlobe[1].price = res.data.config.fortune_globe_money;
					}
				})
				.catch(err => {
		...
		paySubmit() {
			...
			this.orderInfo.subject_id = this.id;
		...
		returnPath() {
			...
			url = `${domain}${this.$api.routesConfig.payComplete.path}?id=${this.id}&type=${this.type}&orderGroup=${this.orderGroup}&params=${this.queryForm}`;
			return url;
		}
	}
};
