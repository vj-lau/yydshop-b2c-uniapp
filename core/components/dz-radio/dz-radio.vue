<template>
	<view class="dz-radio" :style="[radioStyle]">
		<view class="dz-radio__icon-wrap" @tap="toggle" :class="[iconClass]" :style="[iconStyle]">
			<dz-icon
				class="dz-radio__icon-wrap__icon"
			    name="checkbox"
			    :size="elIconSize" 
				:color="iconColor"/>
		</view>
		<view class="dz-radio__label" @tap="onClickLabel" :style="{
...
<script>
	 ...
	 * @example <dz-radio :label-disabled="false">内容</dz-radio>
	 */
	export default {
		name: "dz-radio",
		props: {
		...
		computed: {
			...
			iconStyle() {
				...
				style.width = this.$api.components.addUnit(this.elSize);
				style.height = this.$api.components.addUnit(this.elSize);
				return style;
			},
			iconColor() {
			...
			iconClass() {
				let classes = [];
				classes.push('dz-radio__icon-wrap--' + this.elShape);
				if (this.name == this.parentData.value) classes.push('dz-radio__icon-wrap--checked animation-scale-up animation-duration-03');
				if (this.elDisabled) classes.push('dz-radio__icon-wrap--disabled');
				if (this.name == this.parentData.value && this.elDisabled) classes.push(
					'dz-radio__icon-wrap--disabled--checked');
			...
			radioStyle() {
				...
				if (this.parentData.width) {
					style.width = this.$api.components.addUnit(this.parentData.width);
					// #ifdef MP
					...
					// #ifndef MP
					// H5和APP使用flex布局
					style.flex = `0 0 ${this.$api.components.addUnit(this.parentData.width)}`;
					// #endif
				}
		...
		methods: {
			updateParentData() {
				this.getParentData('dz-radio-group');
			},
			onClickLabel() {
			...
			emitEvent() {
				// dz-radio的name不等于父组件的v-model的值时(意味着未选中)，才发出事件，避免多次点击触发事件
				if(this.parentData.value != this.name) this.$emit('change', this.name);
			},
			// 改变组件选中状态
			// 这里的改变的依据是，更改本组件的parentData.value值为本组件的name值，同时通过父组件遍历所有dz-radio实例
			// 将本组件外的其他dz-radio的parentData.value都设置为空(由computed计算后，都被取消选中状态)，因而只剩下一个为选中状态
