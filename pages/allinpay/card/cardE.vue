				...
				>
					<view class="dz-list-item" @tap="checkBankAccount(item)">
						<view class="mid">
							...
							<view class="u-box">
								<text class="name">{{ userInfo.allinpay_eplus_member && userInfo.allinpay_eplus_member.realname }}</text>
...
export default {
	...
	methods: {
		...
		async getBankAccountIndex() {
			await this.$api.http
				.get(this.$api.apiAllinpay.getCardListEplus, {
					params: {
						card_type: 1,
						page: this.page,
		...
		async setAccountDefault(id) {
			...
			await this.$api.http
				.post(
					this.$api.apiAllinpay.setDefaultWithdrawEplus,
					{},
					{
						params: {
							id: id
		...
		async allinpayBindcardCodeSubmit() {
			...
			await this.$api.http
				.post(this.$api.apiAllinpay.bindCardApplyEplus, { ...postData })
				.then(res => {
		...
		async allinpayBindCardSubmit() {
			...
			this.allinpaySubmitting = true;
			await this.$api.http
				.post(this.$api.apiAllinpay.bindCardAgreeEplus, { ...postData })
				.then(res => {
					this.allinpayAddBankcardShow = false;
					this.allinpaySubmitting = false;
					this.allinpayBankcard = '';
