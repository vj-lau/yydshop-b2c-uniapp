...
export default {
	...
	methods: {
				...
				.then(res => {
					...
					if (this.$api.helper.h5IsWechat() && this.$api.shopSetting.wechat_appid != '') {
						this.$api.wxHelper.wxShare(
							...
							this.$api.shopSetting.bargain_share_title,
							this.productDetail.name,
							this.sharePath(),
					...
					// #endif
					//初始化小程序分享
					this.mpShare.title = this.$api.shopSetting.bargain_share_title;
					this.mpShare.path = this.sharePath();
					this.mpShare.imageUrl = this.productDetail.covers[0];
					//初始化APP分享
					this.shareData = {
						shareUrl: this.sharePath(),
						shareTitle: this.$api.shopSetting.bargain_share_title,
		...
		sharePath() {
			...
			// #ifdef MP
			url = `${this.$api.routesConfig.bargainProduct.path}?id=${this.productId}`;
			// #endif
			// #ifndef MP
			url = `${domain}${this.$api.routesConfig.bargainProduct.path}?id=${this.productId}`;
			// #endif
			if (this.$api.store.getters.hasLogin) {
