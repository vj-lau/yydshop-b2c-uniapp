<template>
	<view
		class="dz-card"
		@tap.stop="click"
		:class="{ 'dz-border': border, 'dz-card-full': full, 'dz-card--border': borderRadius > 0 }"
		:style="{
	...
	>
		<view
			...
			:class="{
				'dz-border-bottom': headBorderBottom
			}"
			@tap="headClick"
		>
			<view v-if="!$slots.head" class="dz-flex dz-row-between">
				<view class="dz-card__head--left dz-flex dz-line-1" v-if="title">
					<image
						:src="thumb"
						class="dz-card__head--left__thumb"
						mode="aspectfull"
						v-if="thumb"
						:style="{ 
					...
					></image>
					<text
						class="dz-card__head--left__title dz-line-1"
						:style="{
				...
				<view class="dz-card__head--right dz-line-1" v-if="subTitle">
					<text
						class="dz-card__head__title__text"
						:style="{
		...
		<view @tap="bodyClick" class="dz-card__body" :style="[{padding: padding + 'rpx'}, bodyStyle]"><slot name="body" /></view>
		<view
			v-if="showFoot"
			class="dz-card__foot"
			 @tap="footClick"
 ...
 * @example <dz-card padding="30" title="card"></dz-card>
 */
export default {
	name: 'dz-card',
	props: {
