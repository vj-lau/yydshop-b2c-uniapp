<template>
	<view class="dz-subsection" :style="[subsectionStyle]">
		<view class="dz-item dz-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'dz-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="dz-item-text dz-line-1">{{ item.name }}</view>
		</view>
		<view class="dz-item-bg" :style="[itemBarStyle]"></view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} mode 模式选择，见"模式选择"说明（默认button）
	 ...
	 * @example <dz-subsection active-color="#ff9900"></dz-subsection>
	 */
	export default {
		name: "dz-subsection",
		props: {
		...
		computed: {
			...
			noBorderRight() {
				return index => {
					...
					let classs = '';
					// 不显示右边的边框
					if (index < this.list.length - 1) classs += ' dz-none-border-right';
					// 显示整个组件的左右边圆角
					if (index == 0) classs += ' dz-item-first';
					if (index == this.list.length - 1) classs += ' dz-item-last';
					return classs;
				};
			},
			// 文字的样式
		...
		methods: {
			...
			getTabsInfo() {
				...
				for (let i = 0; i < this.list.length; i++) {
					view.select('.dz-item-' + i).boundingClientRect();
				}
				view.exec(res => {
					if (!res.length) {
						setTimeout(() => {
