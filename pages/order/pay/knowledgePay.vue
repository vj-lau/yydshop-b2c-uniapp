<template>
	<view class="pay-content">
		<dz-navbar :title="language.orderPay.titleOrderPay"></dz-navbar>
		...
		<view v-if="fortune_globe_pay == 2" class="dz-flex dz-m-l-24 dz-m-r-24">
			<view
				...
				:style="{ background: index == active ? theme.dzBaseColor : theme.dzBgColor, color: index == active ? '#fff' : '#333' }"
			>
		...
		<view class="pay-type-list">
			...
			<dz-cell-item
				...
				"
				center
				:title="language.orderPay.brokeragePay"
				:label="
		...
		<!-- <view class="dz-text-center dz-p-b-30 dz-font-24"><dz-checkbox v-model="agreeCheck" shape="circle" :active-color="theme.dzBaseColor">	</dz-checkbox>已经阅读并且同意用户充值协议</view> -->
		<view class="footer">
			<view class="footer-btn">
				<view style="width: 90%;">
					...
					>
						<!-- {{
							parseInt(orderDetail.order_status) === 0 && parseInt(orderDetail.close_left_time) > 0
								? language.orderPay.confirmPay + language.application.moneySymbol + money
								: language.orderPay.orderClosed
						}} -->
						{{ language.orderPay.confirmPay + language.application.moneySymbol + money }}
					</dz-button>
				</view>
...
export default {
	...
	async onLoad(e) {
		this.money = this.fortuneGlobe[0].price;
		this.orderGroup = this.fortuneGlobe[0].orderGroup;
		this.apiUrl = this.$api.apiShop.knowledgePay;
	},
	onShow() {},
	methods: {
		quickClick(index) {
			this.active = index;
			this.money = this.fortuneGlobe[index].price;
			this.orderGroup = this.fortuneGlobe[index].orderGroup;
		},
		async initOrder() {
			await this.$api.http
				.post(this.$api.apiShop.knowledgeConfig, {
					id: this.id
				})
				.then(res => {
					this.orderGroup = 'knowledge';
					this.fortune_globe_pay = res.data.config.knowledge_globe_pay_mode;
					if (res.data.config.knowledge_globe_pay_mode == 0) {
						this.money = res.data.knowledge.price;
					} else if (res.data.config.knowledge_globe_pay_mode == 1) {
						this.money = res.data.config.knowledge_globe_money;
						this.orderGroup = 'knowledge_globe';
					} else {
						this.money = this.fortuneGlobe[0].price = res.data.knowledge.price;
						this.fortuneGlobe[1].price = res.data.config.knowledge_globe_money;
					}
				})
				.catch(err => {
		...
		paySubmit() {
			...
			this.orderInfo.knowledge_id = this.id;
			this.orderInfo.money = this.money;
		...
		returnPath() {
			...
			url = `${domain}${this.$api.routesConfig.payComplete.path}?id=${this.id}&type=${this.type}&orderGroup=${this.orderGroup}`;
			return url;
		}
	}
};
