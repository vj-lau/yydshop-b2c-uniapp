...
class websocket{
	...
	initWebsocket() {
		...
		// 连接webSocket
		uni.connectSocket({ url: $mAppConfig.websocketUrl });
		...
		uni.onSocketOpen(res => {
			...
			this.send(
				JSON.stringify({ route: $mWebsocketConfig.login, token: $mStore.state.accessToken })
			);
		...
		uni.onSocketError(res => {
			...
			if(this.error>20){
				console.log('超过最大连接次数，停止连接')
				return
			}
			setTimeout(()=>{
				console.log('正在重新连接websocket服务器')
				this.initWebsocket()
			},10*1000)
		});
		
		uni.onSocketClose(res => {
			console.log('websocket连接已关闭！'+JSON.stringify(res));
			this.conncted=false
			this.login=false
			this.error=this.error+1
			if(this.interval){
	...
	send(data) {
		if(!this.conncted){
			console.log('还没有连接上websocket服务器，无法发送消息')
		...
		$mHelper.log('发送websocket数据：'+JSON.stringify(data))
	}
	
	//关闭
	closeWebSocket(){
		if(!this.conncted){
			console.log('还没有连接上websocket服务器，无法关闭连接')
			return
		}
		uni.closeSocket({
			success:()=>{
	...
	websocketOnMessage(e){
		...
		$mHelper.log('接收数据：'+JSON.stringify(e));
		const _this = this;
		let params = {};
						...
						content = data.data.content.replace(
							...
							function(r) {
								if (!_this.emojiToText(r)) {
									...
									return '<img src="'+$mAppConfig.h5Domain+'/static/emoji/'+_this.emojiToText(r)+'.png"></img>';
								}
							}
						);
						break;
