<template>
	<view>
		...
		<view class="dz-form">
			<dz-form>
				<dz-form-item :label="language.modifyMobile.labelMobile" label-width="130">
					<view class="dz-flex">
						<view class="dz-flex-3" @click="areaClick"><input :disabled="true" v-model="areaCodeLabel" /></view>
						<view class="dz-flex-9"><input type="text" v-model="mobile" :disabled="loading" :placeholder="language.modifyMobile.mobileHolder" /></view>
					</view>
				</dz-form-item>
				<dz-form-item :label="language.modifyMobile.labelCode" label-width="130">
...
export default {
	...
	methods: {
		...
		submit() {
			...
			if (!this.$api.helper.isMobile(this.areaCode + this.mobile, this.mobileArea)) {
				this.$refs.dzToast.show({
					title: this.$api.language.modifyMobile.mobileError,
					type: 'warning'
				});
				return;
			}
			...
			this.$api.http
				.post(this.$api.apiShop.mobileReBind, {
					...formData
				})
				.then(res => {
					this.loading = false;
