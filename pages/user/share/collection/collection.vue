<template>
	<view>
		<dz-navbar :title="'我的收藏'"></dz-navbar>
		<view class="content">
			<block v-for="(item, index) in shareContentList" :key="index">
				<view class="card" @tap="tapDetail(item)">
					...
					<view class="tip">{{ item.description || item.title }}</view>
					<!-- 图片 -->
					<view class="image" v-if="item.content_type == 1">
						<view v-for="(items, indexs) in item.cover" :key="indexs" @tap="previewImgs(items, item.cover)">
...
export default {
	...
	async onLoad() {
		uni.setNavigationBarTitle({ title: this.$api.language.discover.titleDiscover });
	...
	methods: {
		async getShareContent() {
			...
			await this.$api.http
				.get(this.$api.apiShare.shareCollection, {
					params: {
						page: this.page,
		...
		copyShareLink(item) {
			...
			let url = `${domain}${this.tabbarList[2].pagePath}?id=${item.id}`;
			if (this.hasLogin) {
		...
		handleCollectDel(index) {
			this.$api.http
				...
				.then(res => {
					...
					this.shareContentList.splice(index, 1);
					if (this.shareContentList.length == 0) {
						this.loadingStatus = 'noData';
					}
				});
		},
